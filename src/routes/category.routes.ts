...
router.post('/create', passport.authenticate('jwt', { session: false }),
  checkRoles('admin'), async (req, res) => {
    try {
...
router.get('/find', passport.authenticate('jwt', { session: false }), checkRoles('admin', 'client'), async (req, res) => {
  try {
    await CategoryService.findCategory()
    res.json({
      success: true, message: '', data: await CategoryService.findCategory()
    })
  } catch (error) {
    res.json({
      success: false, message: 'Error al buscar categorias'
    })
  }
  res.end()
})
